...
test("shows exact source and redistribution state without inventing results", async ({ page }) => {
  ...
  await expect(page.getByText("4 of 4 pinned benchmark records · 0 published trajectories")).toBeVisible();
...
test("filters records and remains readable at mobile width", async ({ page }) => {
  ...
  await expect(page.getByText("2 of 4 pinned benchmark records · 0 published trajectories")).toBeVisible();
